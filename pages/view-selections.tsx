...
const ViewSelections = () => {
    ...
    useEffect(() => {
        const fetchSubmission = async () => {
            if (session?.user?.email) {
                try {
                    const response = await fetch(`/api/get-user-submission?email=${encodeURIComponent(session.user.email)}`);
                    if (response.ok) {
    ...
    return (
        <div className="p-4">
            ...
            <div className="card">
                <h2>Selected on: {new Date(submission.submittedAt).toLocaleString()}</h2>
